"""Task 1: Canny edge extraction on lecturer-provided testing images.
...
def extract_canny_features(image):
    """Return a 4,096-value Canny vector and its edge visualisation."""
...
def valid_canny_vector(features):
    return (
        ...
        and np.count_nonzero(features) > 0
    )
...
def create_display(image, cropped_sign, gray, edge_image, filename, category):
    panels = cv2.hconcat([
...
def main():
    ...
    print("=" * 68)
    print("TASK 1: CANNY EXTRACTION ON LECTURER TESTING IMAGES")
    print("=" * 68)
    ...
    for index, image_path in enumerate(test_paths):
        ...
        try:
            ...
            if not valid_canny_vector(features):
                raise ValueError("invalid or empty Canny edge vector")
    ...
    if not feature_list:
        raise RuntimeError("No valid Canny features were extracted.")
    ...
    print(f"Canny features per image         : {X.shape[1]}")
    print(f"Canny feature matrix shape       : {X.shape}")
