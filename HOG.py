"""Task 1: HOG extraction on training, validation and testing images."""
...
def extract_hog(image, create_visualisation=False):
    """Extract a 4,356-value HOG vector from one traffic-sign image."""
...
def process_partition(partition, records, display_count):
    """Extract HOG features from every record in one partition."""
    ...
    if not feature_list:
        raise RuntimeError(f"No valid HOG features for {partition}.")
...
def main():
    ...
    print("=" * 72)
    print("TASK 1: HOG EXTRACTION ON TRAINING, VALIDATION AND TESTING DATA")
    print("=" * 72)
